bpquant <- function(protein_sig, pi_not, threshold = 1e-8){
...
############# generate parameters associated with signature counts and probabilities #############
  ...
  .x$sig_str <- apply(protein_sig, 1, paste, collapse = "")
  ...
  counts = table(.x$sig_str)
  sigs$count = as.numeric(counts)[match(names(counts), sigs$sig_str)]
  
  ## order signatures by count##
  cnt.ord = counts[ order(counts, decreasing = T)]
  if(uniq_sigs > 1){sig.ord = sigs[match(names(cnt.ord),sigs$sig_str),]}else{sig.ord = sigs}
  
  ## set zero signature first ##
  z.sig = which(apply(abs(sig.ord[,-(ncol(sig.ord) - 0:1)]),1,sum)==0)
  if(length(z.sig) == 0) {
    ...
    pi_probs = c(pi_not, rep((1 - pi_not)/(uniq_sigs - 1), uniq_sigs - 1))
  }
  
  ## identify which unique signature each row matches ##
  counts_ids = match(.x$sig_str, sig.ord$sig_str)
  ...
  # mat = as.matrix(expand.grid(replicate(nu, 0:1, simplify = FALSE)))[,nu:1]
...
#   	}  
  ...
  x_mat[,2] = pbinom(cnt.ord-1, n_peps, pi_probs)
  x_mat[,1] = 1 - x_mat[,2]
  
  ## calculate posterior distribution for each proteoform configuration##
  post_prob <- compute_posterior(pi_probs, x_mat, nu, tol = 1e-50)
  # post_prob = rep(1, n_combos)
  # for (j in 1:n_combos){
  #   x = p_configs[j,]
  #   for(k in 1:nu){
  ...
  post_prob$post_prob = post_prob$post_prob/sum(post_prob$post_prob)
  
  ## store which proteoform configuration has highest posterior probability ##
  id.max = post_prob$index[which.max(post_prob$post_prob)]
  
  ## pull maximum configuration ##
  tmp = (id.max %/% 2^(1:nu - 1)) %% 2
...
return(list(post_prob = post_prob, peptide_idx = peptide_ids, unique_sigs = sigs, num_proteoforms = num_proteoforms))
